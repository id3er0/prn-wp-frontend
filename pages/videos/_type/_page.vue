...
export default {
  ...
  head() {
    const title = objectValue(this, `pages.videos.${this.type}.meta.title`);
    const name = objectValue(this, 'header.name', '');
    const description = objectValue(this, `pages.videos.${this.type}.meta.description`);
    return {
      title: `${title} - ${this.page} | ${name}`,
      meta: [
